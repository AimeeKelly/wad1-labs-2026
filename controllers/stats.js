...
const stats = {
  createView(request, response) {
    ...
    let largestPlaylistSize = Math.max(...playlists.map(playlist => playlist.songs.length));
    let largestPlaylist = playlists.filter(playlist => playlist.songs.length === largestPlaylistSize);
    let largestPlaylists = largestPlaylist.map(item => item.title);

    const statistics = {
        displayNumPlaylists: numPlaylists,
        displayNumSongs: numSongs,
        displayAverage: average,
        displayAvgRating: avgRating.toFixed(2), highest: maxRating,
        displayFav: favTitles,
        displayLargestPlaylistSize: largestPlaylistSize,
        displayLargestPlaylists: largestPlaylists
    };
